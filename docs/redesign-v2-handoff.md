...
1. **OGP画像の軽量化**: 完了・本番反映済み。めいき選定で「コース進捗ダッシュボード」モチーフ（学習特化5案の案B）+ コピー「必要な学びに、すぐ届く。」+ 太字化（ヒラギノ優先指定。Noto Sans JP が拾われると太ウェイトが細く出るため、og-export.html のみ Hiragino 先頭のフォントスタック）。`public/images/ogp.png` は 2400x1260・約204KB（旧3.6MBの約6%）。metadata宣言（app/page.tsx / app/about/page.tsx の 2400x1260）は変更不要。
...
1. **/design-sync push**: インタラクティブターミナルで `claude` → `/design-sync design-system/ をプロジェクト「臨床学術WG Design System」に同期して`（増分sync・dry-run確認後）。対象は design-system/ 配下17ファイル（README・guidelines・tokens.css・foundations 4・components 7・templates 4・og-export.html）。

ogp.png 差し替えと main への push（=Vercelデプロイ）は 2026-07-17 にめいき承認のうえ実施済み。本メモは /design-sync が済んだら削除してよい。
