...
import { LegalDetailGrid, LegalPage, LegalSection } from "../components/legal-page";
  ...
  return (
    ...
    >
      <LegalSection
        id="terms-purpose"
        index="01"
        lead="本サイトは教育用アーカイブの整理と導線提供を目的とし、診療判断の代替を意図しません。"
        title="目的"
      >
      ...
      <LegalSection
        id="terms-conditions"
        index="02"
        lead="公開環境で運用される前提で、公開してよい情報だけを扱い、運営を妨げる行為を禁止します。"
        title="利用条件"
      >
        <ul>
      ...
      <LegalSection
        ...
        index="03"
        lead="動画再生や問い合わせ機能では、配信や不正利用対策のために外部サービスを利用します。"
        title="外部サービス"
      >
        <ul>
      ...
      <LegalSection
        id="terms-disclaimer"
        index="04"
        lead="掲載情報や外部リンクの継続性を保証せず、故意又は重過失を除き責任を負いません。"
        title="免責"
      >
        <ul>
      ...
      <LegalSection
        id="terms-removal"
        index="05"
        lead="権利侵害や掲載ミスなどの問題がある場合は、必要に応じて修正、非公開化、削除を行います。"
        title="修正・削除対応"
      >
      ...
      <LegalSection
        id="terms-updates"
        index="06"
        lead="規約は必要に応じて更新され、日本法に基づいて解釈されます。"
        title="規約変更と準拠法"
      >
      ...
      <LegalSection
        id="terms-operator"
        index="07"
        lead="運営主体、窓口、関連ページを一覧で確認できるようにしています。"
        title="運営者情報"
      >
        <LegalDetailGrid
          items={[
            ...
            { label: "所在地", value: siteLegal.location },
            {
              ...
              value: (
                <>
                  <a href={siteLegal.privacyUrl}>プライバシーポリシー</a> / <a href="/archives">勉強会アーカイブ一覧</a>
                </>
              )
            }
          ]}
